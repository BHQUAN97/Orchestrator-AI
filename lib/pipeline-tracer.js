...
class PipelineTracer extends EventEmitter {
  constructor(options = {}) {
    ...
    this.logDir = options.logDir || null;  // Neu set → ghi trace ra file (legacy)
    // Trace-store: persistent + query-friendly (opt-in via projectDir)
    this.projectDir = options.projectDir || null;
    this.traceStore = null;
    if (options.traceStore) {
      this.traceStore = options.traceStore;
    } else if (this.projectDir) {
      try { this.traceStore = new TraceStore({ projectDir: this.projectDir }); }
      catch { /* silent — logging must never crash pipeline */ }
    }
  ...
  finish(trace, finalResult = {}) {
    ...
    // Ghi trace ra file neu co logDir (legacy path)
    if (this.logDir) {
      this._writeTraceLog(trace.traceId, summary);
    }

    // Auto-persist vao trace-store — query-friendly
    if (this.traceStore) {
      try {
        this.traceStore.save(this._toStoreShape(trace, summary));
      } catch {
        // Silent — persistence failure must not break pipeline
      }
    }
  ...
  /**
   * Export trace full JSON — uu tien doc tu trace-store (co step detail),
   * fallback ve in-memory summary neu chua persist.
   */
  exportTrace(traceId) {
    if (this.traceStore) {
      const persisted = this.traceStore.get(traceId);
      if (persisted) return persisted;
    }
    const trace = this.traces.get(traceId);
    if (!trace) return null;
    const summary = trace.getSummary();
    return this._toStoreShape(trace, summary);
  }

  /**
   * Chuyen Trace + summary thanh shape cua TraceStore
   */
  _toStoreShape(trace, summary) {
    const statusMap = { done: 'completed', failed: 'failed', running: 'running' };
    return {
      id: trace.traceId,
      startedAt: trace.startTime,
      endedAt: trace.endTime || Date.now(),
      status: statusMap[summary?.status] || statusMap[trace.status] || 'running',
      task: trace.metadata?.prompt || trace.operation || '',
      steps: trace.steps.map(s => ({
        agent: s.label || s.name,
        model: s.model,
        tool: s.name,
        input: s.meta || null,
        output: s.result || null,
        cost: s.cost || 0,
        tokens: s.result?.tokens || 0,
      ...
      totalCost: trace.steps.reduce((a, s) => a + (s.cost || 0), 0),
      totalTokens: trace.steps.reduce((a, s) => a + (s.result?.tokens || 0), 0),
      result: summary
    };
  }
