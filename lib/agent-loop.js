...
const { SessionReflector } = require('./session-reflect');
...
// Escalation ladder khi stuck: cheap → smart → architect
// Chỉ leo thang 1 bậc mỗi lần stuck để tránh tốn Opus ngay từ đầu
...
class AgentLoop {
  constructor(options = {}) {
    ...
    // Session reflector — trích lessons từ tool call history sau task_complete
    // Fire-and-forget, không block main flow nếu LiteLLM không có
    this._reflector = new SessionReflector({
      litellmUrl: this.litellmUrl,
      litellmKey: this.litellmKey,
    ...
    // Tool call log — dùng bởi SessionReflector, reset mỗi run()
    this._toolCallLog = [];
  }
  ...
  async run(systemPrompt, userPrompt) {
    ...
    // Reset per-run learning trackers
    this._escalationCount = 0;
    this._lastEscalatedModel = null;
    this._recentTools = [];
    this._iterationCount = 0;
    this._taskPrompt = userPrompt;
    this._toolCallLog = []; // Reset per task — SessionReflector dùng log này
    ...
    // Auto-save memory on task completion (success or fail)
    if (this.memoryAutoSave && this.memoryStore) {
      const completedSuccessfully = this.completed && !this.aborted;
      const summary = this._getCompletionSummary();
      if (summary || !completedSuccessfully) {
        this.memoryStore.append({
          type: 'lesson',
          outcome: completedSuccessfully ? 'success' : 'fail',
          prompt_summary: userPrompt.slice(0, 300),
          summary: String(summary || this.abortReason || 'task_incomplete').slice(0, 500),
          files_changed: this.executor.filesChanged ? [...this.executor.filesChanged] : [],
          iterations: this.iteration,
          model: this.model,
          model_used: this.model || this.config?.model || 'unknown',
          model_final: this._lastEscalatedModel || null,
          cost_usd: this.budget?.spentUsd || 0,
          confidence: this._calcConfidence()
        });
      }

      // Save tool_pattern if a meaningful tool sequence led to completion
      if (completedSuccessfully && this._recentTools && this._recentTools.length >= 2) {
        const toolSequence = [...this._recentTools];
        this.memoryStore.append({
          type: 'tool_pattern',
          summary: `Successful tool pattern: ${toolSequence.join(' → ')}`,
          prompt_summary: userPrompt.slice(0, 200),
          tools_used: toolSequence,
          outcome: 'success',
    ...
    // Non-blocking reflection — best effort, never awaited
    if (this._reflector) {
      this._reflector.reflect({
        taskPrompt: this._taskPrompt || userPrompt,
        toolCallHistory: this._toolCallLog || [],
        outcome: (this.completed && !this.aborted) ? 'success' : 'fail',
  ...
  async _runLoop(tools) {
    ...
    while (this.iteration < this.maxIterations && !this.completed && !this.aborted) {
      ...
      try {
        ...
        // Local model fail (context overflow, connection, no fallback) → silent cloud fallback
        if (this.model.startsWith('local') && _isLocalModelError(e.message)) {
          const fallback = 'smart';
          if (this.onError) this.onError(`[local→${fallback}] ${e.message.slice(0, 100)}`);
          this.model = fallback;
          try {
            response = this.streaming
              ? await this._callLLMStreaming(tools)
            ...
            if (this.onError) this.onError(`LLM call failed: ${e2.message}`);
            this.consecutiveErrors++;
            if (this.consecutiveErrors >= this.maxConsecutiveErrors) { this.aborted = true; break; }
            this.messages.push({ role: 'user', content: `[System] LLM call failed: ${e2.message}. Hãy thử lại.` });
            continue;
          }
        } else {
  ...
  async _executeSingleToolCall(tc) {
    ...
    // Track recent tools for tool_pattern learning (keep last 5)
    if (!this._recentTools) this._recentTools = [];
    if (toolName !== 'task_complete') {
      this._recentTools.push(toolName);
      if (this._recentTools.length > 5) this._recentTools.shift();
    }
    ...
    // Log tool call cho SessionReflector (capped at 20, oldest dropped)
    if (toolName !== 'task_complete') {
      const parsed = safeParse(result.content);
      const callSuccess = !(parsed && parsed.success === false);
      const callError = (!callSuccess && parsed?.error) ? String(parsed.error).slice(0, 100) : undefined;
      ...
      if (this._toolCallLog.length > 20) this._toolCallLog.shift();
    }
    ...
    // Inject stuck warning + escalate model (da detect TRUOC execute nhung push sau result)
    if (stuckWarning) {
      // Escalate model truoc khi inject message — agent moi co the break pattern
      const escalated = MODEL_ESCALATION[this.model];
      if (escalated && escalated !== this.model) {
        const fromModel = this.model;
        const toModel = escalated;
        const reason = stuckWarning.message.slice(0, 80);
        this.model = toModel;
        this._escalationCount = (this._escalationCount || 0) + 1;
        this._lastEscalatedModel = toModel;
        if (this.onRouting) this.onRouting({ from: fromModel, to: toModel, decision: { method: 'stuck-escalation', reasons: [reason] } });
        this.messages.push({ role: 'user', content: `[System] Stuck detected — escalating model ${fromModel} → ${toModel}. ${stuckWarning.message}` });

        // Save escalation event to memory
        if (this.memoryStore) {
          try {
            this.memoryStore.append({
              type: 'model_escalation',
              prompt_summary: this._taskPrompt?.slice(0, 200) || this._lastUserPromptSample?.slice(0, 200) || '',
              summary: `Task required escalation from ${fromModel} to ${toModel}: ${reason}`,
              from_model: fromModel,
              to_model: toModel,
              escalation_reason: reason,
              outcome: 'escalated',
              model_used: fromModel,
              model_final: toModel,
              tags: [`#${fromModel}-fail`, `#${toModel}-success`]
            });
          } catch { /* silent */ }
        }
      } else {
    ...
    if (this.selfHealer) {
      try {
        ...
        if (suggestion) {
          // Inject suggestion ngay vào messages hiện tại (không chờ iteration sau)
          // để agent có thể adjust approach trong cùng session
          this.messages.push({
            role: 'user',
            content: `[Self-healer] ${suggestion.message}`
          });
          // Không cần pendingHealerSuggestion nữa vì đã inject trực tiếp
        }
      } catch { /* silent */ }
    }
  ...
  // =============================================
  // LOCAL MODEL — truncate context de tranh n_keep > n_ctx crash
  // =============================================

  /**
   * Khi dung local model, cat system message xuong LOCAL_SYS_LIMIT chars.
   * Local model chi co 4096-8192 token context — system prompt day du qua lon.
   * Non-mutating: tra ve copy moi, khong modify messages goc.
   */
  _truncateForLocal(messages) {
    if (!this.model.startsWith('local')) return messages;
    const LOCAL_SYS_LIMIT = 6000; // ~1500 tokens — con 2596 token cho conversation + output
    const sysIdx = messages.findIndex(m => m.role === 'system');
    if (sysIdx === -1) return messages;
    const sys = messages[sysIdx];
    const content = typeof sys.content === 'string' ? sys.content : '';
    if (content.length <= LOCAL_SYS_LIMIT) return messages;
    const out = [...messages];
    const truncated = content.slice(0, LOCAL_SYS_LIMIT) +
      '\n[... context truncated for local model ...]';
    out[sysIdx] = { ...sys, content: truncated };
    return out;
  }

  // =============================================
  // PRIVACY FILTER — redact sensitive content trước khi gửi LLM
  // =============================================

  /**
   * Sanitize messages: thay thế nội dung nhạy cảm trong tool results bằng placeholder.
   * Áp dụng khi model là cloud và content chứa pattern nhạy cảm.
   * Non-mutating: trả về copy mới, không modify this.messages.
   *
   * Detect patterns: KEY=value, secret/password/token dạng long string,
   * PEM headers, connection string có credentials.
   */
  _sanitizeMessagesForCloud(messages) {
    // Chỉ cần sanitize khi gửi lên cloud (local model nhận raw)
    if (this.model.startsWith('local')) return messages;
    ...
    const out = messages.map(msg => {
      ...
      const redacted = _redactSensitive(content);
      if (redacted === content) return msg;
      changed = true;
  ...
  _lazyLoadEmbeddings() {
    try {
      ...
      return new EmbeddingStore({
        projectDir: this.projectDir,
        endpoint: this.litellmUrl,
        apiKey: this.litellmKey,
      });
    } catch { return null; }
  }
  ...
  async _callLLM(tools) {
    const messagesForCall = this._truncateForLocal(
      this._sanitizeMessagesForCloud(
        await this._applyRagIfNeeded(this.messages)  // non-mutating copy
      )
    );
    ...
    const isLocal = this.model.startsWith('local');
    let body = {
      model: this.model,
      messages: cachedMessages,
      tools: cachedTools,
      tool_choice: 'auto',
      max_tokens: isLocal ? 1024 : (parseInt(process.env.ORCAI_MAX_OUTPUT_TOKENS, 10) || 4096),
      temperature: 0.2
    };
  ...
  /**
   * Tinh confidence score cho task vua hoan thanh.
   * Penalty cho escalations (agent can help) va iterations cao (agent phai thu nhieu lan).
   */
  _calcConfidence() {
    const escalations = this._escalationCount || 0;
    const iterations = this.iteration || this._iterationCount || 0;
    const base = 0.7;
    const escalationPenalty = escalations * 0.15;
    const iterationPenalty = Math.max(0, (iterations - 10) * 0.01);
    return Math.max(0.1, Math.min(1.0, base - escalationPenalty - iterationPenalty));
  }
 ...
 */
/**
 * Redact sensitive patterns trong tool result content trước khi gửi cloud LLM.
 * Patterns: KEY=value dạng env, PEM block, connection string có credentials,
 * token/secret/password assignment với giá trị dài.
 */
const _SENSITIVE_PATTERNS = [
  // .env style: KEY=VALUE (value dài hơn 8 chars để tránh false positive)
  { re: /^([A-Z_]{3,}(?:KEY|SECRET|TOKEN|PASSWORD|PWD|API_KEY|PRIVATE|AUTH|CREDENTIAL)[S]?\s*=\s*).{8,}$/gim,
    replace: '$1[REDACTED]' },
  // PEM certificate/key blocks
  { re: /-----BEGIN [A-Z ]+-----[\s\S]*?-----END [A-Z ]+-----/g,
    replace: '[REDACTED PEM BLOCK]' },
  // Connection strings với credentials: protocol://user:pass@host
  { re: /(\w+:\/\/[^:@\s]+:)[^@\s]{4,}(@)/g,
    replace: '$1[REDACTED]$2' },
  // JSON fields: "token":"...", "secret":"...", "password":"..."
  { re: /("(?:token|secret|password|apiKey|api_key|privateKey|private_key|credential|auth_token|access_token|refresh_token)"\s*:\s*")([^"]{8,})(")/gi,
    replace: '$1[REDACTED]$3' },
];

function _redactSensitive(content) {
  if (typeof content !== 'string' || content.length < 16) return content;
  let out = content;
  for (const { re, replace } of _SENSITIVE_PATTERNS) {
    // Reset lastIndex vì dùng /g flag
    re.lastIndex = 0;
...
/**
 * Detect local model errors that warrant immediate cloud fallback.
 * Don't show these as user errors — silently switch model.
 */
function _isLocalModelError(msg) {
  if (!msg) return false;
  return (
    /n_keep.*n_ctx/i.test(msg) ||
    /context length/i.test(msg) ||
    /No fallback model group.*local/i.test(msg) ||
    /MidStreamFallbackError/i.test(msg) ||
    /APIConnectionError.*local/i.test(msg) ||
    /ECONNREFUSED.*1234/i.test(msg)
  );
}
