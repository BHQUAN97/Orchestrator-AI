...
class AgentLoop {
  ...
  async run(systemPrompt, userPrompt) {
    ...
    while (this.iteration < this.maxIterations && !this.completed && !this.aborted) {
      ...
      for (const tc of toolCalls) {
        ...
        // Check error → increment consecutive errors
  ...
  async continueWith(userMessage) {
    ...
    // Chạy lại loop từ trạng thái hiện tại
    const tools = getTools(this.agentRole);

    while (this.iteration < this.maxIterations && !this.completed && !this.aborted) {
      this.iteration++;

      if (this.onThinking) this.onThinking(this.iteration, this.maxIterations);
      ...
      try {
        ...
        this.messages.push({ role: 'user', content: `[System] Error: ${e.message}` });
        continue;
      }

      const message = response.choices?.[0]?.message;
      if (!message) { this.aborted = true; break; }
      ...
      for (const tc of message.tool_calls) {
        ...
        const result = await this.executor.execute(tc);
        if (this.onToolResult) this.onToolResult(tc.function.name, result);
        this.messages.push(result);

        if (tc.function.name === 'task_complete') this.completed = true;
        const parsed = JSON.parse(result.content);
        if (parsed.success === false) this.consecutiveErrors++;
  ...
  async _callLLMStreaming(tools) {
    ...
    try {
      ...
      // Stream bị đứt giữa chừng → fallback non-streaming
      return this._callLLM(tools);
    }
