  ...
  async _executeSingleToolCall(tc) {
    ...
    if (stuckWarning) {
      ...
      if (escalated && escalated !== this.model) {
        ...
        if (this.onRouting) this.onRouting({ from: prev, to: escalated, decision: { method: 'stuck-escalation', reasons: [stuckWarning.message.slice(0, 80)] } });
        this.messages.push({ role: 'user', content: `[System] Stuck detected — escalating model ${prev} → ${escalated}. ${stuckWarning.message}` });
      } else {
