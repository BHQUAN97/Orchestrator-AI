...
class AgentLoop {
  constructor(options = {}) {
    ...
    // Auto-route: tu dong chon model theo task
    ...
    this._ragOriginalSystemPrompt = null; // snapshot system prompt goc
  ...
  async run(systemPrompt, userPrompt) {
    ...
    // Auto-route model based on task (neu co Hermes bridge + enabled)
    if (this.autoRoute && this.hermesBridge && userPrompt) {
      try {
        ...
        if (decision?.model && decision.model !== this.model) {
          ...
          if (this.onError) this.onError(`[auto-route] ${prev} → ${decision.model} (${decision.method}: ${(decision.reasons || []).join(', ').slice(0, 80)})`);
        } else {
          this.lastRoutingDecision = decision;
        }
      } catch (e) {
        // ignore routing errors — keep configured model
      }
    }

    // Inject relevant past experience vao system prompt (memory recall)
    let enrichedSystem = systemPrompt;
    if (this.memoryStore && userPrompt) {
  ...
  async _executeSingleToolCall(tc) {
    ...
    // Stuck detector — inject warning message neu detect
    if (this.stuckDetector) {
      const stuck = this.stuckDetector.record(toolName, args);
      if (stuck) {
        // Inject system reminder, agent se thay o next iteration
        this.messages.push({
          role: 'user',
          content: `[System reminder] ${stuck.message}`
        });
        // Reset de khong spam warning moi tool call
  ...
  /**
   * Neu model local → rebuild system prompt voi RAG context.
   * Thay this.messages[0] (system) tai cho — khong modify cloud path.
   */
  async _applyRagIfLocal() {
    if (this.ragDisabled) return;
    const builder = this._getRagBuilder();
    if (!builder) return;
    if (!builder.isLocalModel(this.model)) {
    ...
    // Snapshot lan dau de co the re-apply khi model doi
    if (this._ragOriginalSystemPrompt == null) {
      const orig = this.messages[sysIdx].content;
      this._ragOriginalSystemPrompt = typeof orig === 'string' ? orig : '';
    }

    try {
      const enriched = await builder.build({
        basePrompt: this._ragOriginalSystemPrompt,
        userMessage: this._lastUserPromptSample || '',
        modelId: this.model
      });
      this.messages[sysIdx].content = enriched;
    } catch (e) {
