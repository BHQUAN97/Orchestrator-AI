...
class LocalAssistant {
  ...
  _scheduleRebuildIfStale(examplesPath) {
    ...
    process.stderr.write('[local-assist] 🔄 Index stale — rebuilding in background...\n');
    ...
    this._rebuildPromise = new Promise((resolve) => {
      ...
      try {
        ...
        child.on('close', (code) => {
          clearTimeout(timer);
          const ok = code === 0;
          if (ok) process.stderr.write('[local-assist] ✓ Index rebuilt\n');
          else process.stderr.write(`[local-assist] ⚠ Rebuild exit ${code}\n`);
          finish(ok);
        });
        child.on('error', (e) => {
          clearTimeout(timer);
          process.stderr.write(`[local-assist] ⚠ Rebuild spawn error: ${e.message}\n`);
          finish(false);
        });
      } catch (e) {
        clearTimeout(timer);
        process.stderr.write(`[local-assist] ⚠ Cannot spawn rebuild: ${e.message}\n`);
        finish(false);
      }
    });
  }
  ...
  async _summarize(content, filePath, query) {
    ...
    const prompt = `You are a code summarizer. Your ONLY job is to summarize the contents of the file below.
DO NOT answer the query. DO NOT provide solutions or suggestions.
