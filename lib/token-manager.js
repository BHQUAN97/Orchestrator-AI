...
// Tokenizer chinh xac — uu tien tiktoken (gpt-tokenizer), fallback heuristic
let tokenizer;
try {
  tokenizer = require('./tiktoken-counter');
} catch {
  tokenizer = null;
}

// Fallback: ~4 ký tự = 1 token (trung bình cho tiếng Anh + code)
...
class TokenManager {
  ...
  estimateTokens(messages) {
    ...
    // Prefer accurate tiktoken-based count
    if (tokenizer && tokenizer.countMessagesTokens) {
    ...
    for (const msg of messages) {
      totalChars += 16; // role + structural overhead
  ...
  /**
   * Check xem dang dung tokenizer chinh xac hay heuristic
   */
  get tokenizerName() {
    return tokenizer?.encoderName || 'heuristic';
  }
