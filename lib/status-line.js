...
const { getRateLimitState } = require('./retry');
...
function renderStatusLine(agent, opts = {}) {
  ...
  // Rate limit (only if we have data + low remaining)
  try {
    const rl = getRateLimitState();
    if (rl.lastError && Date.now() - rl.lastError.ts < 60000) {
      parts.push(chalk.red(`rl!`));
    } else if (rl.remaining != null && rl.limit != null) {
      const pct = Math.round((rl.remaining / rl.limit) * 100);
      if (pct < 20) parts.push(chalk.red(`rl ${rl.remaining}/${rl.limit}`));
      else if (pct < 50) parts.push(chalk.yellow(`rl ${pct}%`));
    }
  } catch {}
