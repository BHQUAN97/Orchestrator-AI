...
class SelfHealer {
  constructor({ memoryStore = null, enabled = true } = {}) {
    ...
    this._recentToolCalls = [];     // rolling buffer of last 5 tool names (for tool chain context)
  }
  ...
  observe(toolName, args, result) {
    ...
    // Track rolling tool call history (last 5 tools before any error)
    this._recentToolCalls.push(toolName);
    if (this._recentToolCalls.length > 5) this._recentToolCalls.shift();
    ...
    if (parsed.success === false || parsed.error) {
      ...
      if (entry.lastError === errorKey) {
        ...
        entry.count = 1;
        entry.firstAt = now;
        // Capture tool chain at start of new error streak (exclude current failing tool)
        entry.toolChain = this._recentToolCalls.slice(0, -1).slice(-3);
      }
  ...
  _saveGotcha(toolName, args, entry) {
    ...
    if (this.memoryStore) {
      try {
        // Classify error type for richer tagging
        const errorType = _classifyErrorType(entry.errorText, entry.count);
...
/**
 * Classify error type based on error text and count pattern.
 * Used for richer gotcha tagging and analysis.
 */
function _classifyErrorType(errorText, count) {
  if (!errorText) return 'unknown';
  const lower = errorText.toLowerCase();
  if (/timeout|timed out/i.test(lower)) return 'timeout';
  if (/permission denied|eacces|enoent/i.test(lower)) return 'permission';
  if (/not found|no such file|enoent/i.test(lower)) return 'not_found';
  if (/toggle|flip|alternating/i.test(lower)) return 'toggle';
  if (count >= REPEAT_THRESHOLD) return 'repeat';
  return 'generic';
}
