...
/**
 * Loi throw khi hard cap bi vuot — caller abort execution
 */
class BudgetExceededError extends Error {
  constructor(message, details = {}) {
    super(message);
    this.name = 'BudgetExceededError';
    this.capType = details.capType;      // 'daily' | 'task'
    this.currentUSD = details.currentUSD;
    this.projectedUSD = details.projectedUSD;
...
/**
 * Enforce hard cap truoc khi chay LLM call.
 * Throws BudgetExceededError neu cost-tracker block.
 *
 * @param {string} taskId
 * @param {number} projectedCostUSD
 * @param {string} [projectDir] - optional, default process.cwd()
 * @returns {{allowed: true}} neu pass
 */
function enforceHardCap(taskId, projectedCostUSD, projectDir) {
  // Lazy require de tranh circular
  const { getCostTracker } = require('./cost-tracker');
  const tracker = getCostTracker(projectDir);
  const result = tracker.checkCap({ taskId, projectedCostUSD });
  if (!result.allowed) {
    throw new BudgetExceededError(result.reason, {
...
module.exports = { BudgetTracker, computeCost, lookupPrices, MODEL_PRICES, BudgetExceededError, enforceHardCap };
