...
class WorktreeSession {
  ...
  create(baseBranch) {
    ...
    // Validate baseBranch — chỉ cho phép ref name hợp lệ (tránh shell injection dù đã dùng execFileSync)
    if (!/^[A-Za-z0-9._/\-~^]+$/.test(base)) {
      throw new Error(`Invalid baseBranch: ${base}`);
    }
  ...
  cleanup(opts = {}) {
    ...
    try {
      execFileSync('git', ['worktree', 'remove', '--force', this.worktreePath], {
        cwd: this.projectDir,
        stdio: ['ignore', 'pipe', 'pipe'],
        timeout: 10000
      });
    ...
    if (opts.deleteBranch && this.branch) {
      try {
        execFileSync('git', ['branch', '-D', this.branch], {
          cwd: this.projectDir,
          stdio: ['ignore', 'pipe', 'pipe'],
          timeout: 5000
        });
