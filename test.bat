...
node -e "const{SmartRouter}=require('./router/smart-router');const r=new SmartRouter({availableModels:['gemini-flash','kimi-k2.5','deepseek','sonnet']});[{t:'build',f:['src/c.tsx'],p:'Fix button'},{t:'build',f:['src/s.service.ts'],p:'Add API'},{t:'review',f:['src/a.ts'],p:'Review code'},{t:'spec',f:[],p:'Design feature'}].forEach(x=>{const m=r.route({task:x.t,files:x.f,prompt:x.p});console.log(x.p.padEnd(20)+' -> '+m.model)})"
echo.
