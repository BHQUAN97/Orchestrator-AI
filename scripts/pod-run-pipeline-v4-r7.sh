...

{
  ...
  echo "[stage 1/3] TRAIN lora v4-r7 (Qwen 2.5 Coder 7B, rank=$R6_LORA_RANK epochs=$R6_EPOCHS seq=$R6_MAX_SEQ_LEN fresh=$R6_FRESH_START)"
  cd "$ROOT" && python3 scripts/train-lora-qwen7b-v4-r7.py
  TRC=$?
  if [ $TRC -ne 0 ]; then
  ...
  echo "===== $(date -u +%FT%TZ) ====="
  echo "[stage 2/3] MERGE adapter v4-r7"
  cd "$ROOT" && python3 scripts/merge-lora-7b-v4-r7.py
  MRC=$?
  if [ $MRC -ne 0 ]; then
  ...
  echo "===== $(date -u +%FT%TZ) ====="
  echo "[stage 3/3] CONVERT merged-v4-r7 to GGUF Q4_K_M"
  apt-get update -q && apt-get install -y -q cmake build-essential git >/dev/null 2>&1
  cd "$ROOT" && bash scripts/convert-to-gguf-v4-r7.sh
  CRC=$?
  if [ $CRC -ne 0 ]; then
  ...
  ls -lh "$OUT/qwen7b-ft-v4-r7-Q4_K_M.gguf" 2>&1
